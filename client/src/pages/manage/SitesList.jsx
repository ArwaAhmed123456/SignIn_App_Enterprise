...
const SitesList = () => {
  ...
  const [selectedSite, setSelectedSite] = useState(null); // null = list view
  ...
  // If a site is selected, show the settings view
  if (selectedSite) {
    return (
      <div>
        <button onClick={() => setSelectedSite(null)} className="flex items-center gap-1.5 text-sm text-[#2b4594] hover:underline font-semibold mb-6">
        ...
        <SiteSettings
          ...
          onDeleted={() => { setSelectedSite(null); fetchSites(); }}
        />
      </div>
    );
  }
        ...
        <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden">
          {sites.map(site => (
            <div key={site.id} onClick={() => setSelectedSite(site)}
              ...
              <span className="text-sm text-slate-400 hidden group-hover:flex items-center gap-1.5">
                → Devices &amp; posters
              </span>
            </div>
          ))}
