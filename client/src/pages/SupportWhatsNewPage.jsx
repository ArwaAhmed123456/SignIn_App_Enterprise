    ...
    <div className="h-full overflow-auto bg-slate-50">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-8 py-8">
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#eef3fb] text-[#2b4594]">
            <BellDot size={24} />
          </div>
          ...
          {WHATS_NEW_ITEMS.map((item) => (
            <article key={item.slug} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full bg-[#eef3fb] px-3 py-1 text-sm font-medium text-[#2b4594]">
                  Release note
                </span>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm">
          ...
          <Link to="/admin/support/collections/getting-started" className="font-semibold text-[#2b4594] hover:underline">
            Getting Started
          </Link>
          .
        </div>
