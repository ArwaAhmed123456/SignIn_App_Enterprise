...
// ===============================
// Email transporter — Gmail with timeout protection
// ===============================
const createTransporter = () => nodemailer.createTransport({
    host: 'smtp.gmail.com',
    port: 465,
    secure: true,          // port 465 = SSL, more reliable on cloud hosts
    auth: {
        user: process.env.EMAIL_USER,
        pass: process.env.EMAIL_PASSWORD,
    },
    connectionTimeout: 10000,   // 10s — prevents 502 server crash
...
// ===============================
// Send any email with auto-retry on failure
// ===============================
const sendMail = async (mailOptions, retries = 2) => {
    for (let i = 0; i <= retries; i++) {
        try {
            const transporter = createTransporter(); // fresh connection each time
...
const sendPasswordResetEmail = async (email, projectName, resetToken) => {
    return sendMail({
        ...
        html: `
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#fff;border-radius:12px;border:1px solid #e5e7eb;">
  <h2 style="color:#2b4594;margin:0 0 16px;">Password Reset Request</h2>
  <p style="color:#374151;">You requested a password reset for <strong>${projectName}</strong>.</p>
  <div style="background:#f0f4ff;border:2px solid #2b4594;border-radius:10px;padding:24px;text-align:center;margin:24px 0;">
    <p style="color:#64748b;font-size:13px;margin:0 0 8px;text-transform:uppercase;letter-spacing:1px;">Your code</p>
  ...
  <p style="color:#6b7280;font-size:13px;">Expires in 15 minutes. If you didn't request this, ignore this email.</p>
</div>`
    });
};
...
const sendPreRegistrationInviteEmail = async ({ email, name, siteName, expectedDate, notes }) => {
    ...
    return sendMail({
        ...
        html: `
<div style="font-family:Arial,sans-serif;padding:24px;border:1px solid #e5e7eb;border-radius:12px;max-width:560px;">
  ...
  <div style="background:#f8fafc;border-radius:10px;padding:16px;margin:16px 0;">
    <p><strong>Expected arrival:</strong> ${visitLabel}</p>
...
const sendMobileInviteEmail = async (guard, plainToken) => {
    ...
    return sendMail({
        from: process.env.EMAIL_FROM || `"Sign In App" <${process.env.EMAIL_USER}>`,
        to: guard.email,
        subject: `Your mobile activation code`,
        html: `
<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#fff;border-radius:12px;border:1px solid #e5e7eb;">
  ...
  <div style="background:#0f172a;border-radius:12px;padding:24px;text-align:center;margin:24px 0;">
    ...
    <p style="color:#94a3b8;font-size:12px;margin:10px 0 0;">Expires in 72 hours</p>
  </div>
  <p style="color:#6b7280;font-size:13px;">Open the companion app → tap Activate → enter this code.</p>
</div>`
    });
};

// ===============================
// Welcome email — matches Sign In App reference design
// ===============================
const sendWelcomeEmail = async ({ email, name, groupName, siteName, orgName, companionCode }) => {
    ...
    <tr><td style="padding:0 40px 32px;">
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">Your account has been granted permission to use the Sign In App companion app. Sign in and out of the premises from your phone.</p>
      <table width="100%" cellpadding="0" cellspacing="0"><tbody>
        <tr>
          ...
          <td style="padding-left:12px;padding-bottom:14px;font-size:14px;color:#374151;"><strong>Download</strong> the companion app from the App Store or Play Store and open it.</td>
        </tr>
        <tr>
          <td style="width:32px;vertical-align:top;padding-top:2px;">
          ...
          <td style="padding-left:12px;padding-bottom:14px;font-size:14px;color:#374151;"><strong>Enter your 12-digit code</strong> below and tap Connect.</td>
        </tr>
        <tr>
          <td style="width:32px;vertical-align:top;padding-top:2px;">
          ...
          <td style="padding-left:12px;font-size:14px;color:#374151;"><strong>Done!</strong> You're ready to sign in from your phone.</td>
        </tr>
      </tbody></table>
      <p style="font-size:13px;color:#6b7280;margin:20px 0 10px;">Your companion app code (valid for 72 hours):</p>
...
<table width="100%" style="max-width:600px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);" cellpadding="0" cellspacing="0"><tbody>
  ...
  <tr><td style="padding:0 40px 32px;">
    <div style="background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;padding:24px;text-align:center;">
      ...
      <p style="color:#64748b;font-size:14px;margin:0 0 20px;">The QR Code below will be scanned at the iPad when you arrive to quickly recover your details.</p>
  ...
  <tr><td style="padding:0 40px 32px;">
    <div style="background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;padding:18px 20px;">
      <p style="color:#166534;font-size:14px;margin:0;"><strong>✅ You are all set!</strong> Please don't delete this email. When you arrive, scan the QR code above.</p>
    </div>
  </td></tr>
  ...
  <tr><td style="padding:20px 40px 32px;text-align:center;border-top:1px solid #f3f4f6;">
    ...
    <p style="color:#cbd5e1;font-size:12px;margin:4px 0 0;">© ${new Date().getFullYear()} All rights reserved.</p>
  </td></tr>
